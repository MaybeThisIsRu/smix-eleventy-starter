module.exports = {
    postUrl: (collection, slug) => {
        try {
            ...
            if (found === 0 || found === undefined) throw `${slug} not found in specified collection.`
            else return found.url
        } catch (e) {
            console.error(`An error occured while searching for the url to ${slug}. Details:`, e)
        }
    },
    isOldPost: (date) => {
        ...
        const cutoffTimestamp = getUnixTime(new Date().setFullYear(new Date().getFullYear() - 1))
